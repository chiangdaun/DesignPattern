...
然后我们分别基于该抽象类实现管理员显示类 AdminShow 和用户显示类 UserShow ，这样一来我们就使操作和使用这些操作的客户端完全分开了。
在最后我们重新实现了 Question ，并且 Question.show 方法直接调用显示对象的显示方法。
这样一来我们将 Question 对象和显示方法进行了解耦，增加新的显示方法时，只需要增加新的显示对象就可以了。
同时，在代码中还可以看到我们可以动态改变 Question 的显示方式，这也体现了策略模式的互换行为。
"""
